...
def main():
    ...
    arm = fr3.get_planning_component("arm")
    logger.info("MoveItPy instance created")

    # Use the Pilz Industrial Motion Planner (PTP - point-to-point) for the
    # single-pipeline plans below. "pilz_ptp" is the parameter namespace
    # defined in config/motion_planning_api.yaml.
    pilz_ptp = PlanRequestParameters(fr3, "pilz_ptp")
    ...
    # set goal state using predefined state
    arm.set_goal_state(configuration_name="extended")

    # plan to goal
    plan_and_execute(fr3, arm, logger, single_plan_parameters=pilz_ptp, sleep_time=3.0)
    ...
    # randomize the robot state (of the 'arm' group only, so it stays valid)
    ...
    joint_constraint = construct_joint_constraint(
        ...
        joint_model_group=fr3.get_robot_model().get_joint_model_group("arm"),
    )
    ...
    multi_pipeline_plan_request_params = MultiPipelinePlanRequestParameters(
        fr3, ["ompl_rrtc", "pilz_ptp", "chomp_planner"]
    )
